...
const bookingOptions = document.querySelectorAll(".booking-choice");
...
function getPriceNumber(option) {
    ...
    const dataPrice = parseInt(option.dataset.price, 10);
...
function getServiceLabel(option) {
    if (option.dataset.gift === "true") {
...
function updateBookingSummary() {
    ...
    const selectedOptions = document.querySelectorAll(".booking-choice.selected");

    bookingSummaryList.innerHTML = "";

    if (selectedOptions.length === 0) {
        bookingSummaryList.innerHTML = "<li>Aucun service sélectionné</li>";
    ...
    selectedOptions.forEach((option) => {
        const serviceName = getServiceLabel(option);
...
if (bookingButton) {
    bookingButton.addEventListener("click", () => {
        ...
        let services = [];
        let total = 0;
        let hasGiftCardWithoutAmount = false;

        selectedOptions.forEach((option) => {
            const price = getPriceNumber(option);

            if (option.dataset.gift === "true" && price <= 0) {
                hasGiftCardWithoutAmount = true;
            }

            services.push(getServiceLabel(option));
            total += price;
        });
        ...
        if (selectedOptions.length === 0) {
            alert("Merci de choisir au moins un service ou une carte cadeau.");
            return;
        }

        if (hasGiftCardWithoutAmount) {
            alert("Merci d’indiquer le montant de la carte cadeau.");
            return;
        }

        if (!date || !time || !name) {
            alert("Merci de choisir une date, une heure et d’indiquer votre nom.");
            return;
        }
        ...
        const whatsappMessage =
            `Bonjour I-Motion Dakar,%0A%0A` +
            `Je souhaite faire une demande de réservation.%0A%0A` +
            `Sélection :%0A${servicesText}%0A%0A` +
